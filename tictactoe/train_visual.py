"""Self-play training loop with live visualization dashboard."""

from tic_tac_toe_env import TicTacToeEnv
from q_learning_agent import QLearningAgent
from evaluate import evaluate_greedy, plot_learning_curve, plot_final_stats
from visualization import LiveTrainingDashboard
...
# Visualization update frequencies
DEMO_GAME_EVERY = 500
LEARNING_CURVE_EVERY = 500  # Update curve every episode for smooth scrolling
OPENING_HEATMAP_EVERY = 500  # Opening heatmaps don't change much
...
def train():
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        if episode % LEARNING_CURVE_EVERY == 0:
            dashboard.update_learning_curve(history)
            dashboard.update_opening_heatmaps(agent_x, agent_o)

        if episode % DEMO_GAME_EVERY == 0:
            first_player = 1 if demo_count % 2 == 0 else 2
            dashboard.play_demo_game(env, agent_x, agent_o, episode, first_player)
            demo_count += 1

        # Light refresh on curve updates that didn't trigger a demo
        if episode % LEARNING_CURVE_EVERY == 0 and episode % DEMO_GAME_EVERY != 0:
            dashboard.refresh()

    # Keep dashboard open after training
    print("\nTraining complete. Close the dashboard window to continue...")
    plt.ioff()
    plt.show()

    # Greedy evaluation + static plots (same as train.py)
